...
namespace LeaveRequestService.Services
{
    public class LeaveRequestManagementService : ILeaveRequestManagementService
    {
        ...
        public async Task<PaginatedResultDto<LeaveRequestReadDto>> GetAllAsync(
       ClaimsPrincipal requester, string? searchTerm, string? status,
       int pageNumber, int pageSize)
        {
            var requesterId = int.Parse(requester.FindFirstValue(ClaimTypes.NameIdentifier)!);
            int? singleUserId = null;
            List<int>? userIds = null;

            if (requester.IsInRole("Admin"))
            {
                // Admin có thể xem tất cả, không cần lọc theo userIds
                userIds = null;
            }
            else if (requester.IsInRole("SuperUser"))
            {
                var superUserDepartment = requester.FindFirstValue("Department");
                // Gọi sang UserService để lấy ID của các user trong bộ phận
                userIds = await _userService.GetUserIdsByDepartmentAsync(superUserDepartment);
            ...
            {
                singleUserId = requesterId;
            }

            // Gọi Repository với các tham số đã được xác định
            var (leaveRequests, totalCount) = await _repository.GetAllAsync(status, userIds, singleUserId, pageNumber, pageSize);

            if (!leaveRequests.Any())
            {
                return new PaginatedResultDto<LeaveRequestReadDto> { TotalCount = 0 };
            }

            // "Làm giàu" dữ liệu bằng cách gọi sang UserService
            var allUserIdsInPage = leaveRequests.Select(r => r.UserId).Distinct().ToList();
            var userDetailsMap = await _userService.GetUserDetailsByIdsAsync(allUserIdsInPage);

            var resultDtos = leaveRequests.Select(req =>
            {
                ...
                return dto;
            }).AsEnumerable(); // Chuyển về IEnumerable

            // Lọc theo searchTerm ở service vì nó chứa dữ liệu từ UserService
            if (!string.IsNullOrWhiteSpace(searchTerm))
            {
                var lowerSearchTerm = searchTerm.ToLowerInvariant();
                resultDtos = resultDtos.Where(dto =>
            ...
            return new PaginatedResultDto<LeaveRequestReadDto>
            {
                Items = resultDtos.ToList(), // Chuyển về List để trả về
        ...
        public async Task<byte[]> ExportLeaveRequestsToExcelAsync(ClaimsPrincipal requester, string? searchTerm = null, string? status = null)
        {
            // Bước 1: Gọi hàm GetAllAsync với pageNumber = 1 và pageSize = int.MaxValue để lấy tất cả dữ liệu
            var paginatedResult = await GetAllAsync(
                requester,
                searchTerm,
                status,
                1,          // pageNumber
            ...
            var leaveRequestsToExport = paginatedResult.Items;

            // Bước 2: Tạo file Excel (không cần set LicenseContext ở đây nữa)
            using (var package = new ExcelPackage())
            {
                ...
                var headers = new[]
                {
            "Mã NV", "Họ tên", "Email", "Bộ phận", "Chức vụ", "Vai trò",
            "Loại phép", "Từ ngày", "Đến ngày", "Giờ từ", "Giờ đến",
            "Lý do", "Trạng thái", "Lý do xử lý", "Ngày tạo"
        };
                ...
                // Đổ dữ liệu vào từ hàng thứ 2
                if (leaveRequestsToExport.Any())
                {
                    worksheet.Cells["A2"].LoadFromCollection(
                        leaveRequestsToExport.Select(item => new
                        {
                            item.MaSoNhanVien,
                            item.HoTen,
                            item.Email,
                            ...
                            item.ChucVu,
                            item.Role,
                            LoaiPhep = Enums.EnumHelper.GetDisplayName(item.LoaiPhep),
                            NgayTu = item.NgayTu.ToString("dd/MM/yyyy"),
                            NgayDen = item.NgayDen.ToString("dd/MM/yyyy"),
                            item.GioTu,
                            item.GioDen,
                            item.LyDo,
                            TrangThai = Enums.EnumHelper.GetDisplayName(item.TrangThai),
                            item.LyDoXuLy,
                            NgayTao = item.NgayTao.ToString("dd/MM/yyyy HH:mm")
                        }),
                        false // Không in header của object
                    );
                }
        ...
        public async Task<LeaveRequestDashboardStatsDto> GetDashboardStatsAsync(ClaimsPrincipal requester)
        {
            ...
            // SỬA LẠI: Gọi hàm GetAllAsync với đầy đủ tham số để lấy tất cả đơn
            if (isAdmin || isSuperUser)
            {
                // Admin/SuperUser lấy tất cả đơn (không lọc theo user)
                var result = await _repository.GetAllAsync(null, null, null, 1, int.MaxValue);
                overviewRequests = result.Requests;
            }
            else
            {
                // User thường chỉ lấy đơn của chính mình
                var result = await _repository.GetAllAsync(null, null, requesterId, 1, int.MaxValue);
                overviewRequests = result.Requests;
            }
            ...
            var userDetailsMap = await _userService.GetUserDetailsByIdsAsync(overviewUserIds);
        ...
        public async Task<PaginatedResultDto<UserDetailDto>> GetAllUsersWithLeaveDetailsAsync(
    ...
    int pageSize)
        {
            PaginatedResultDto<UserDetailDto> paginatedUsers;
            try
            {
                var client = _httpClientFactory.CreateClient("UserClient");
                var token = _httpContextAccessor.HttpContext?.Request.Headers["Authorization"].ToString();
                ...
                // Bước 1: Gọi sang UserService để lấy danh sách user ĐÃ ĐƯỢC PHÂN TRANG
            ...
            $"pageSize={pageSize}"
                ...
                paginatedUsers = await client.GetFromJsonAsync<PaginatedResultDto<UserDetailDto>>(url);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Lỗi khi gọi UserService để lấy danh sách user.");
                throw; // Ném lỗi ra để frontend biết
            }

            if (paginatedUsers == null || !paginatedUsers.Items.Any())
            {
                return new PaginatedResultDto<UserDetailDto>(); // Trả về kết quả rỗng
            }

            // Bước 2: Chỉ lấy ID của các user trong trang hiện tại
            var userIdsOnPage = paginatedUsers.Items.Select(u => u.Id).ToList();
            var currentYear = DateTime.UtcNow.Year;

            // Bước 3: Lấy tất cả đơn nghỉ phép đã duyệt trong năm của những user đó
            var relevantLeaveRequests = await _repository.GetApprovedLeaveForUsersAsync(userIdsOnPage, currentYear);

            var leaveDaysTakenByUser = relevantLeaveRequests
                .GroupBy(r => r.UserId)
                .ToDictionary(g => g.Key, g => g.Sum(r => r.DurationInDays));

            // Bước 4: "Làm giàu" dữ liệu ngày phép vào kết quả
            foreach (var userDto in paginatedUsers.Items)
            {
                double totalEntitlement = 12.0; // Có thể lấy từ userDto nếu có
