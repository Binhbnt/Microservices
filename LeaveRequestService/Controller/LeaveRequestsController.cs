...
public class LeaveRequestsController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<PaginatedResultDto<LeaveRequestReadDto>>> GetLeaveRequests(
        [FromQuery] string? searchTerm,
        [FromQuery] string? status,
        [FromQuery] int pageNumber = 1,
        [FromQuery] int pageSize = 10)
    {
        var result = await _leaveRequestService.GetAllAsync(User, searchTerm, status, pageNumber, pageSize);
        return Ok(result);
    }
    ...
    [FromQuery] string? searchTerm = null, // Sửa tên tham số `search` thành `searchTerm` cho nhất quán
    [FromQuery] string? status = null)   // Sửa tên tham số `role` thành `status`
    {
        try
        {
            // Truyền đúng tham số xuống service
        ...
        {
            _logger.LogError(ex, "Lỗi khi xuất Excel cho LeaveRequests.");
    ...
    [FromQuery] int pageSize = 10)
    {
        var result = await _leaveRequestService.GetAllUsersWithLeaveDetailsAsync(
            User, isDeleted, searchTerm, role, pageNumber, pageSize
        );
        return Ok(result);
