...
namespace UserService.Services
{
    public class UserManagementService : IUserManagementService
    {
        ...
        public async Task<PaginatedResultDto<UserReadDto>> GetAllAsync(
       ...
       int pageNumber, int pageSize)
        ...
        {
            // Bước 1: Lấy tất cả dữ liệu người dùng phù hợp với bộ lọc (không phân trang)
            // Chúng ta truyền requesterRole và requesterDepartment là null vì hàm export không cần phân quyền dữ liệu phức tạp
            var (usersToExport, _) = await _userRepository.GetAllAsync(
                searchTerm, role, isDeleted,
                "Admin", // Giả định quyền Admin để lấy hết dữ liệu
                null,    // Không lọc theo phòng ban cụ thể
                1,       // Trang 1
                int.MaxValue // Lấy tất cả các mục
            );

            // Bước 2: Tạo file Excel bằng EPPlus
            // ExcelPackage.LicenseContext đã được cấu hình ở Program.cs hoặc appsettings.json
            using (var package = new ExcelPackage())
            {
                var worksheet = package.Workbook.Worksheets.Add("Users");

                // Thêm tiêu đề cột (Header Row)
                worksheet.Cells["A1"].Value = "Mã số NV";
                worksheet.Cells["B1"].Value = "Họ tên";
                worksheet.Cells["C1"].Value = "Email";
                worksheet.Cells["D1"].Value = "Chức vụ";
                worksheet.Cells["E1"].Value = "Bộ phận";
                ...
                using (var range = worksheet.Cells["A1:G1"])
                {
                    ...
                    // Lưu ý: Dòng SetColor này có thể gây lỗi trên Linux nếu chưa cấu hình đúng
                    // Nếu bạn gặp lỗi 500 khi export, hãy comment dòng này lại
                ...
                {
                    // Dùng hàm LoadFromCollection để đổ dữ liệu nhanh hơn
                    worksheet.Cells["A2"].LoadFromCollection(
                        usersToExport.Select(u => new
                        {
                            u.MaSoNhanVien,
                            u.HoTen,
                            u.Email,
                        ...
                        false // không in header vì đã tự thêm ở trên
                    );
